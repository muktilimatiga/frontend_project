...
const databases = [
  ...
  { id: 'db-3', name: 'legacy_archived_2023', type: 'MongoDB', region: 'ap-northeast-1', size: '16GB / 2vCPU', lastSynced: '2 months ago', status: 'offline', tables: 12, storage: 95, color: 'text-slate-500 bg-slate-100 dark:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-700' },
    ...
    <div className="space-y-6">
       <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-zinc-400">Manage your cloud database instances and clusters.</p>
          </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {databases.map((db) => (
             <Card 
                key={db.id} 
                className="group hover:border-indigo-300 dark:hover:border-indigo-800 transition-all cursor-pointer dark:bg-zinc-900 dark:border-zinc-800 hover:shadow-md"
             ...
             >
                <CardContent className="p-6">
                         ...
                         <div>
                            <h3 className="font-bold text-slate-900 dark:text-white text-base leading-tight">{db.name}</h3>
                             <div className="flex items-center gap-2 mt-1.5">
                                ...
                                <span className="text-xs text-slate-500 dark:text-zinc-400 font-medium">• {db.type}</span>
                             </div>
                         </div>
                      </div>
                      <DropdownMenu>
                   ...
                   <div className="grid grid-cols-2 gap-y-5 gap-x-4 mb-6">
                      <div>
                         <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-zinc-500 mb-1">
                            <Globe className="h-3.5 w-3.5" /> Region
                         </div>
                         <p className="text-sm font-semibold text-slate-700 dark:text-zinc-200">{db.region}</p>
                      </div>
                      <div>
                         <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-zinc-500 mb-1">
                            <Cpu className="h-3.5 w-3.5" /> Size
                         </div>
                         <p className="text-sm font-semibold text-slate-700 dark:text-zinc-200">{db.size}</p>
                      </div>
                       <div>
                         <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-zinc-500 mb-1">
                            <Activity className="h-3.5 w-3.5" /> Last Synced
                         </div>
                         <p className="text-sm font-semibold text-slate-700 dark:text-zinc-200">{db.lastSynced}</p>
                      </div>
                      <div>
                         <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-zinc-500 mb-1">
                            <Layers className="h-3.5 w-3.5" /> Tables
                         </div>
                         <p className="text-sm font-semibold text-slate-700 dark:text-zinc-200">{db.tables}</p>
                      </div>
                   </div>
          ...
          {/* New Database Card */}
          <div 
             className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-zinc-800 rounded-xl p-6 hover:bg-slate-50 dark:hover:bg-zinc-800/50 cursor-pointer transition-all gap-4 group min-h-[320px]"
          >
             <div className="h-14 w-14 rounded-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/20 transition-colors shadow-sm">
                <Plus className="h-6 w-6 text-slate-400 group-hover:text-indigo-600 dark:text-zinc-500 dark:group-hover:text-indigo-400" />
             </div>
             <div className="text-center">
                <p className="text-lg font-semibold text-slate-900 dark:text-white">New Database</p>
                <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1">Deploy a new instance</p>
             </div>
          </div>
       </div>
    </div>
